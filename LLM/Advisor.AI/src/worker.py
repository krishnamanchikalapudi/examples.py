...
def get_ticker_news(ticker, n_search_results=3):
    ...
    try:
        ...
        links = [
            n["link"] for n in company.news if n["type"] == "STORY"
    ...
    search = GoogleSerperAPIWrapper(
        type="news", tbs="qdr:d5", serper_api_key=os.environ["SERPER_API_KEY"]
    )
    results = search.results(f"financial news about {company} or {ticker}")
    if not results["news"]:
        logger.error(
            f"No search results for the previous 5 days. Let's try to broaden the search range to one week!"
        )
        search = GoogleSerperAPIWrapper(
            type="news", tbs="qdr:w1", serper_api_key=os.environ["SERPER_API_KEY"]
        )
        results = search.results(f"financial news about {company} or {ticker}")
...
def _format_results(docs, query):
    ...
    for doc in docs:
        ...
        page_content = (
            doc.page_content.strip() if query in doc.page_content else ""
...
def get_financial_statements(ticker):
    ...
    return (
        ...
        cash_flow_csv_filename,
        income_statement_csv_filename,
    )
...
def process_prompt(ticker):
    ...
    return {"input": ticker, "text": response}
